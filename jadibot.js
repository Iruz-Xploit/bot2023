...
const { boom, chalk, fs, figlet, FileType, path, process, PhoneNumber } = modul;
...
const jadibot = async (gans, m, from) => {
const { sendImage, sendMessage } = gans;
...
const gans = await makeWaSocket({
auth: state,
browser: [`Jadibot Md By (gans Bot)`, "Chrome", "1.0.0"],
logger: log({ level: "silent" }),
version,
})

gans.ws.on('CB:Blocklist', json => {
...
let qutsnya = await gans.sendContact(callerId, owner)
await gans.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
...
if (!gans.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(gans, kay, store)
...
gans.ev.on("connection.update", async up => {
...
if (connection == "open") {
gans.id = gans.decodeJid(gans.user.id)
gans.time = Date.now()
global.conns.push(gans)
await m.reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× id : ${gans.decodeJid(gans.user.id)}*_`)
user = `${gans.decodeJid(gans.user.id)}`
txt = `*Terdeteksi menumpang Jadibot*\n\n _× User : @${user.split("@")[0]}_`
sendMessage(`6282299284898@s.whatsapp.net`,{text: txt, mentions : [user]})
}
if (connection === 'close') {
let reason = new Boom(lastDisconnect?.error)?.output.statusCode
if (reason === DisconnectReason.badSession) { 
console.log(`Bad Session File, Please Delete Session and Scan Again`); gans.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); gans.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); gans.logout(); }
...
gans.send5ButLoc = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
...
gans.sendButImg = async (jid, path, teks, fke, but) => {
...
gans.sendMessage(jid, fjejfjjjer, { quoted: m })
}
