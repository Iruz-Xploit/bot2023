...
const { default: gansConnect, useMultiFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
const { figlet } = modul;
...
const { smsg, isUrl, generateMessageTag, getBuffer, reSize,getSizeMedia, fetchJson, await, sleep } = require('./lib/myfunc')
...
require('./gans.js')
nocache('../gans.js', module => console.log(color('[ UPDATE ]', 'cyan'), color(`'${module}'`, 'green'), 'File Di Perbarui!!'))
require('./index.js')
nocache('../index.js', module => console.log(color('[ UPDATE ]', 'cyan'), color(`'${module}'`, 'green'), 'File Di Perbarui!!'))
...
browser: ['GansBotz Multi Device','Safari','1.0.0'],
...
// Anti Call
gans.ev.on('call', async (fatihh) => {
let botNumber = await gans.decodeJid(gans.user.id)
...
let pa7rick = await gans.sendTextWithMentions(tihh.from, `*${gans.user.name}* tidak bisa menerima panggilan ${tihh.isVideo ? `video` : `suara`}. Maaf @${tihh.from.split('@')[0]} kamu akan diblockir. Jika tidak sengaja silahkan hubungi Owner untuk dibuka !`)
gans.sendContact(tihh.from, global.owner, pa7rick)
await sleep(8000)
await gans.updateBlockStatus(tihh.from, "block")
}
}
}
})
...
require("./gans")(gans, m, chatUpdate, store)
} catch (err) {
 console.log(err)
}
})
...
// Get Profile Picture Group
   try {
   ppgc = await gans.profilePictureUrl(ciko.id, 'image')
   } catch {
   ppgc = 'https://tinyurl.com/yx93l6da'
   }
   let wm_fatih = { url : ppgc }
   if (ciko.announce == true) {
   gans.send5ButImg(ciko.id, `「 Group Settings Change 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message`, wm_fatih, [])
...
const date = moment.tz('asia/jakarta').format('HH:mm:ss')
const memnya = metadata.participants.length
...
let lea = await new knights.Goodbye()
    ...
    .setBackground("https://telegra.ph/file/4b90043328ec4825c0e71.jpg")
    .toAttachment();
let wah = await reSize(wel.toBuffer(), 300, 150)
let yah = await reSize(lea.toBuffer(), 300, 150)

if (anu.action == 'add') {
gans.sendMessage(anu.id, {
location: { jpegThumbnail: wah } ,
caption: `┌─❖
│「 𝗛𝗶 👋 」
└┬❖ 「 @${num.split("@")[0]}  」
   │✑  𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼 
...
📝 Description: `,
mentions : [num, creator],
footer: `${metadata.desc ? metadata.desc : 'Welcome New Mem'}`,
buttons: [{buttonId: `.gadak`, buttonText: {displayText: `welcome👋`}, type: 1}],
headerType: "LOCATION"
})
} else if (anu.action == 'remove') {
gans.sendMessage(anu.id,  {
location: { jpegThumbnail: yah } ,
caption: `┌─❖
│⟮ 𝗚𝗼𝗼𝗱𝗯𝘆𝗲 👋 ⟯
└┬❖ ⟬ @${num.split("@")[0]} ⟭
   │✑  𝗟𝗲𝗳𝘁 
...
buttons: [{buttonId: `.gadak`, buttonText: {displayText: `Good Bye👋`}, type: 1}],
headerType: "LOCATION"
})
...
let reason = new Boom(lastDisconnect?.error)?.output.statusCode
 ...
 else if (reason === DisconnectReason.Multidevicemismatch) { console.log("Multi device mismatch, please scan again"); gans.logout(); }
 ...
 * @returns 
 */
gans.sendText = (jid, text, quoted = '', options) => gans.sendMessage(jid, { text: text, ...options }, { quoted, ...options })
...
gans.sendKatalog = async (jid , title = '' , desc = '', gam , options = {}) =>{
let message = await prepareWAMessageMedia({ image: gam }, { upload: gans.waUploadToServer })
